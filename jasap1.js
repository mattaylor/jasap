...
//dojo.declare('Jasap', schemaViews, { constructor: function(name, options) {
	...
	my.root	 = options.root ? dijit.byId(options.root) : dijit.byId('root');
	console.log('root:'+ my.root);//my.root  = dijit.byId((typeof options.root === 'object') ? options.root : dijit.byId(options.root) || dijit.byId('root');
...
//		})
	...
	my.load = function(init) {
		init = init || 'init';
		if (!my.user) return my.auth('Mat Taylor', 'free60', function() { my.load(init) } );
	...
	my.rend	=  function(view, object, options) {		// do stuff with views
		...
		var root   = (options && options.root) ? dijit.byId(options.root) :  my.root;
		...
		if (!node) {
			...
			console.log('root:'+root, 'node:'+ node);
			root.addChild(node);
		}
	...
	tabs :  function(options) {  // render views as tabs
		var node = new dijit.layout.TabContainer({title:my.name, nested:true});
		node.addChild(my.views.grid());
		my.root = node;
		return node;
	},
	
	split:	function(options) { 
		var node = new dijit.layout.BorderContainer({title:my.name});
		var root = new dijit.layout.StackContainer({id:my.name+'Stacks', width:'40%', region:'right'});
		var grid = my.views.grid({ width:'60%', region:'center'});
		var form = new dijit.TitlePane({title:'new '+my.name, content:my.views.form()});
		node.addChild(grid);
		node.addChild(root);
		root.addChild(form);
	...
	form : function(object, options) {
		object = object || { }
		options = options || { id:'form'+my.name+(object.id || '') };
		options.content = '<table class="form">';
		if (object.id) options.content += '<input type="hidden" name="id" value="'+ object.id + '"/>';
		for (var key in my.schema.properties) if (!my.schema.properties[key].readOnly) {
			options.content += '<tr><td> '+ key +'</td><td><input name="'+ key +'" value="'+ [key] +'"/>'
		}
	
		var form = new dijit.form.Form(options);			
